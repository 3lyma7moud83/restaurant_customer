...
class _LoginPageState extends State<LoginPage>
  ...
  void _handleBackNavigation() {
    unawaited(_handleBackNavigationInternal());
  }

  Future<void> _handleBackNavigationInternal() async {
    await InputFocusGuard.prepareForUiTransition(context: context);
    if (!mounted) {
      return;
    }
  ...
  void _handleModeChanged(_AuthMode? nextMode) {
    if (_loading || nextMode == null || nextMode == _mode) {
      return;
    }
    unawaited(_switchMode(nextMode));
  }

  Future<void> _switchMode(_AuthMode nextMode) async {
    await InputFocusGuard.prepareForUiTransition(context: context);
    if (!mounted || _loading || nextMode == _mode) {
      return;
    }
                ...
                child: LayoutBuilder(
                  builder: (context, viewportConstraints) {
                    ...
                    final cardPadding = EdgeInsets.all(
                      isCompact ? 16 : (width >= 900 ? 24 : 22),
                    );
                    final segmentHorizontalPadding = isCompact ? 12.0 : 18.0;
                    final contentPadding = EdgeInsets.fromLTRB(
                      basePadding.left,
                      keyboardVisible ? 10 : 16,
                      basePadding.right,
                      (kIsWeb ? 22.0 : 14.0) + (bottomInset * 0.18),
                    );
                                        ...
                                        Directionality(
                                          ...
                                          child:
                                              CupertinoSlidingSegmentedControl<
                                                  _AuthMode>(
                                            groupValue: _mode,
                                            onValueChanged: _handleModeChanged,
                                            children: {
