...
do $system_errors_unique_index$
begin
  if to_regclass('public.idx_system_errors_open_fingerprint') is not null then
    return;
  end if;
  ...
  if exists (
    ...
    group by app_name, fingerprint
    having count(*) > 1
  ) then
    raise notice 'Skipping idx_system_errors_open_fingerprint: duplicate open fingerprints exist in production data.';
    return;
  end if;

  create unique index idx_system_errors_open_fingerprint
    on public.system_errors (app_name, fingerprint)
    where is_resolved = false and fingerprint is not null;
end;
$system_errors_unique_index$;

do $system_errors_policies$
begin
  if not exists (
    select 1
    from pg_policies
    where schemaname = 'public'
      ...
      and policyname = 'allow system error inserts'
  ) then
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'system_errors'
      and policyname = 'allow system error select'
  ) then
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'system_errors'
      and policyname = 'allow system error updates'
  ) then
